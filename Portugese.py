...
try:
    ...
    original_data = pd.read_csv("data/portugese_words.csv")
...
def next_card():
    ...
    selected_card = current_card["Portugese"]
    window.after_cancel(flip_timer)
    canvas.itemconfig(card_title, text="Portugese", fill="black")
    canvas.itemconfig(card_word, text=selected_card, fill="black")
    canvas.itemconfig(card_side, image=card_front_img)
    window.after(100)
    audio_output = gTTS(text=current_card["Portugese"], lang=language)
    audio_output.save("portugese_word.mp3")
    playsound.playsound("portugese_word.mp3", False)
    os.remove("portugese_word.mp3")
...
window = Tk()
window.title("BeeLingual(Vocabulary Building App for Portugese)")
...
card_title = canvas.create_text(400, 150, text="Portugese", font=("Ariel", 40, "italic"))
