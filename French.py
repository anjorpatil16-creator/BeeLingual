...
def is_known():
    if len(to_learn) > 1:
        ...
        messagebox.showinfo(title="Congratulation!",message="There's no words to learn!\nYou've review all the words!\nGood job, keep up the good work!")
...
def next_card():
    ...
    playsound.playsound("french_word.mp3", False)
...
window = Tk()
window.title("BeeLingual(Vocabulary Building App for French)")
window.config(padx=40, pady=40, bg=BACKGROUND_COLOR)
 
flip_timer = window.after(3000, func=flip_card)
 
canvas = Canvas(width=600, height=426)
card_front_img = PhotoImage(file="images/frontcard1.png")
card_back_img = PhotoImage(file="images/frontcard1.png")
card_side = canvas.create_image(300, 263,  image=card_front_img)
card_title = canvas.create_text(200, 180, text="French", font=("Ariel", 29, "italic"))
card_word = canvas.create_text(200, 263, text="word", font=("Ariel", 29, "bold"))
