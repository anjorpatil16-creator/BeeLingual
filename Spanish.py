...
def next_card():
    ...
    selected_card = current_card["Spanish"]
    ...
    window.after(100)
    audio_output = gTTS(text=current_card["Spanish"], lang=language)
    audio_output.save("spanish_word.mp3")
    playsound.playsound("spanish_word.mp3", False)
    os.remove("spanish_word.mp3")
...
window = Tk()
window.title("BeeLingual(Vocabulary Building App for Spanish)")
